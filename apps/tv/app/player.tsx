...
import { TvVideoPlayer } from "@/src/components/player/TvVideoPlayer";
...
export default function PlayerScreen() {
  ...
  const handleProgress = (
    ...
    if (options?.flush) {
      persistProgress(positionSeconds, durationSeconds, options.completed);
      return;
    }

    if (Date.now() - lastSaveAtRef.current >= PLAYBACK_PROGRESS_SAVE_INTERVAL_MS) {
      persistProgress(positionSeconds, durationSeconds, false);
    }
  };
  ...
  const title = params.title ?? playbackData.item.title;
