...
import { useMovieList } from "@/src/hooks/queries";
...
export function MovieListScreen({ title, description, sort }: Props) {
  ...
  if (items.length === 0) {
    return <EmptyState title="No movies" message="No movies match this list." />;
  }
  ...
  return (
    ...
    >
      ...
      <View style={[styles.grid, gridStyle]}>
        {items.map((item) => (
          <MovieCard key={item.id} movie={item} width={itemWidth} onPress={() => router.push(`/movies/${item.id}`)} />
        ))}
      </View>
