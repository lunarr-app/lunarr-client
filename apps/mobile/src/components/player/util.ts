...
export function absolutePlaybackDuration(input: {
  relativeDurationSeconds: number;
  streamStartSeconds?: number | null;
  streamRelativeTimeline: boolean;
}) {
  const relativeDuration = Math.floor(input.relativeDurationSeconds);
  if (!input.streamRelativeTimeline) return relativeDuration;
  const streamStartSeconds =
    Number.isFinite(input.streamStartSeconds) && Number(input.streamStartSeconds) > 0
      ? Number(input.streamStartSeconds)
      : 0;
  return streamStartSeconds + relativeDuration;
}
